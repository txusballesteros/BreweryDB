...
import kotlinx.android.synthetic.main.fragment_search_section_is_organic.*
import javax.inject.Inject

class IsOrganicSearchSectionFragment: SearchSectionFragment(), SeachSectionPresenter.View {
  companion object {
    fun newInstance(): IsOrganicSearchSectionFragment = IsOrganicSearchSectionFragment()
  }

  @Inject lateinit var presenter: SeachSectionPresenter

  override fun onRequestLayoutResourceId(): Int = R.layout.fragment_search_section_is_organic
  ...
  override fun renderSearchQuery(query: SearchQueryViewModel) {
    query.isOrganic?.let { isOrganic.isChecked = query.isOrganic }
  }

  override fun getQuery(source: SearchQueryViewModel): SearchQueryViewModel {
    return source.copy(
        isOrganic = isOrganic.isChecked
    )
  }
}